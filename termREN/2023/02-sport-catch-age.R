...
# Read mapping file -------------------------------
RENmap <- readxl::read_excel(path=paste0(here::here("termREN"), "/", analysis_year, "/", 
                                         list.files(path=paste0(here::here("termREN"), "/", analysis_year),
                                                    pattern="^TERMREN_mapping_[0-9]{4}\\.xlsx$",
                                                    full.names=F)),   
...
RENrecCatchbyAge <- full_join(SCrecCatch %>%
...
pdf(file = here::here("termREN", "2023", "figures", 
                      paste0("Recreational fishery age composition ", 
                             min(RENrecCatchbyAge$YEAR), "-", max(RENrecCatchbyAge$YEAR), 
                             " (Terminal Renfrew areas).pdf")),   
    width = 11, # The width of the plot in inches
    height = 8.5) # The height of the plot in inches

ggplot() +
  geom_bar(data=RENrecCatchbyAge %>% 
             filter(propn>0),
  ...
  geom_text(data=RENrecCatchbyAge %>% 
              group_by(YEAR, MONTH) %>% 
...
# --> THERE HAS NEVER REALLY BEEN A CASE WHERE ANY GIVEN MONTH HAS A GOOD SAMPLE RATE FOR AREA 20
#     THEREFORE, ages are pooled ACROSS ALL SUB-AREAS/MONTHS WITHIN A YEAR  


# Pooling samples based on rules above -------------------------------
RENrecCatchbyAge_pooled <- RENrecCatchbyAge %>%
  group_by(YEAR) %>%
  mutate(
...
RENmap01 <- left_join(RENmap %>% 
                      ...
                      RENrecCatchbyAge_pooled %>% 
                        ...
                        filter(if_else(YEAR%in%RENmap$TermRun_Year, TRUE,  YEAR==(YEAR-1))) %>%
...
writexl::write_xlsx(RENrecCatchbyAge_pooled, 
                    path=paste0(here::here("termREN"), "/", analysis_year, "/", "supplementary",
                                "/R_OUT - Recreational fishery age sample pooling details (Terminal Renfrew areas) ",
                                analysis_year,
                                "-output_from_01",
                                ".xlsx"))
